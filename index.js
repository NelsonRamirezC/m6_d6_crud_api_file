...
app.get("/api/v1/personas", async (req, res) => {
    try {
        ...
        res.status(200).send({ code: 200, data: personas, message: "OK" });
    } catch (error) {
...
app.put("/api/v1/personas/:id", async (req, res) => {
    try {
        ...
        let { nombre, apellido, email } = req.body;
        //terminar código
        let personaModificada = new Persona({ id, nombre, apellido, email });
        let respuesta = await personaModificada.update();
